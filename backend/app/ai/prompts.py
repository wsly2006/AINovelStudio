...
def _chapter_summary_block(chapter: Chapter) -> str:
    """本章大纲块。是写正文 / 续写时优先级最高的硬约束,尤其是结局裁决。"""
    text = (chapter.summary or "").strip()
    if not text:
        return ""
    # 网文训练分布让模型天然爱「留反派活口 / 留续集钩子」,
    # 这里显式把「结局照写」和「不许自创道具」抬到硬约束级别。
    return (
        "本章大纲(硬约束,优先级高于任何写作套路或续集钩子):\n"
        f"{text}\n"
        "硬性要求:\n"
        "- 大纲点名的事件必须全部真实发生,顺序可微调,结局不可改。\n"
        "- 大纲对结局的裁决(谁死 / 谁活 / 谁得到什么 / 谁逃走 / 结局伤势)"
        "必须原样兑现:把「全歼」写成「重创后逃走」、把「身亡」写成「昏迷被救」、"
        "让大纲写死的死者活下来搜寻线索——都算违规。\n"
        "- 禁止追加大纲未提到的关键要素(法宝、传讯道具、援兵、隐藏身份、"
        "临终托孤、续集悬念钩子等),即使这是网文常见套路也不行。\n"
        "- 若大纲没写「留悬念」,就按大纲写死的结局收尾,不要为了钩子而扭曲结局。"
    )
