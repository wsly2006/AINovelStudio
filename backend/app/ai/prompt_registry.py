...
_WRITING_SYSTEM = (
    ...
    "禁止在开头输出章节编号或标题(如「第一章」「# 第一章 xxx」),"
    "章节标题会在导出时另行拼接,正文直接从第一段故事文字开始。"
)
...
请严格按「本章大纲」推进本章情节,把大纲里列出的每一个事件、动机、冲突和结局裁决全部落到正文;承接上一章结尾,贴合总纲走向、推动主线进展。
特别提醒:大纲的结局(谁死 / 谁活 / 谁得到什么 / 结局伤势)是硬约束,不要因为「留悬念更好读」而软化;也不要追加大纲未写的道具、援兵、传讯、隐藏身份等要素。{{extra_instruction_block}}"""
...
续写要贴合「本章大纲」里剩余尚未展开的部分,尤其是大纲写死的结局裁决(谁死 / 谁活 / 谁得到什么);不要偏离本章设定的情节走向,也不要追加大纲未提到的道具、援兵、传讯或续集钩子。
